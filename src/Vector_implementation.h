...
template <typename T>
void Vector<T>::copyFrom(T const* A, Rank lo, Rank hi) {	//以数组A[lo,hi)为蓝本复制向量
...
void Vector<T>::expand() {
	if (size_ < capacity_) return;						//尚未满员时，无需扩容
...
template <typename T>
void Vector<T>::shrink() {								//装填因子过小时，压缩
	if (capacity_ < DEFAULT_CAPACITY << 1) return;		//不至于收缩到DEFAULT_CAPACITY一下
	if (size_ << 2 > capacity_) return;					//以25%为界	
	T* oldElem = elem_;
...
template <typename T>
void permute(Vector<T>& V) { 						//随机置乱算法
	for (int i = V.size(); i >  0; i--) {			//自后向前  采用自后向前的原因为容易在[0,i)之间生成随机数
...
//此处 直接通过下标访问内部数组的元素
template <typename T>
void Vector<T>::unsort(Rank lo, Rank hi) {			//区间置乱算法
...
Rank Vector<T>::find(T const& e, Rank lo, Rank hi) const {
	assert(0 <= lo && lo <= hi && hi <= size_);
	while( (lo < hi--) && e != elem_[hi]);	//从后向前，顺序查找
	return hi;
}
...
int Vector<T>::remove(Rank lo, Rank hi) {		//删除区间[lo,hi)
	assert(0 <= lo && lo <= hi && hi <= size_);
	if (lo == hi) return 0;						//处于效率考虑，单独处理退化情况，例如remove(0,0)
	while (hi < size_) {
...
{
	while (1 < hi - lo) {	//每步迭代只要做一次判断比较，有两个分支,成功查找前不能停止
		Rank mi = (lo + hi) >> 1;	//以中点为轴点
...
void Vector<T>::merge(Rank lo, Rank mi, Rank hi)
{
	...
	int lb = mi - lo;
	T* B = new T[lb];
	for (Rank i = 0; i < lb; B[i] = A[i++]);//复制前子向量B[0,lo) = elem_[lo, hi)
	int lc = hi - mi;
	T* C = elem_ + mi;
	//将B[j]和C[k]中小者续至Am末尾
